...
export default function ProductCard({ product }: { product: any }) {
  ...
  return (
    <div 
      className="group/card w-full relative flex flex-col h-full bg-white rounded-2xl border border-gray-100 shadow-[0_2px_10px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.08)] transition-all duration-300 overflow-hidden"
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
    >
      <div className="relative aspect-[4/5] bg-[#F8F9FA] overflow-hidden">
        <Link href={`/product/${productId}`} className="flex items-center justify-center w-full h-full p-4">
          <img 
