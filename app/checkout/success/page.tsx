        ...
        <div className="max-w-3xl mx-auto">
          <div className="bg-white border border-teal-100 rounded-[2.5rem] p-8 md:p-12 shadow-2xl shadow-teal-900/5 overflow-hidden">
            ...
            <div className="text-center mb-10">
              <div className="w-16 h-16 bg-[#E0F7FA] rounded-full flex items-center justify-center mx-auto mb-6">
                <CheckCircle2 size={32} className="text-[#5BCDE9]" />
              </div>
            ...
            {loading ? (
                ...
                <div className="md:col-span-3 space-y-8">
                  ...
                  <div className="bg-[#1A2E35] rounded-3xl p-8 text-white relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                      <p className="text-[10px] text-[#5BCDE9] uppercase tracking-[0.3em] font-black mb-2">
                        {t("order_number_label")}
                      </p>
                      <p className="text-4xl font-black tracking-tighter mb-4">
                        {order.order_number}
                      </p>
                      <div className="h-1 w-12 bg-[#5BCDE9] rounded-full mb-4" />
                      <p className="text-xs text-gray-400 font-bold">
                        {t("confirmation_sent_to")}: <span className="text-white">{order.customer_email}</span>
                      </p>
                    </div>
                  </div>

                  {/* Delivery Status */}
                  <div className="flex items-center gap-5 text-sm font-bold text-[#1A2E35] bg-[#E0F7FA] border border-[#B2EBF2] p-6 rounded-3xl">
                    <div className="w-10 h-10 bg-white rounded-2xl flex items-center justify-center shadow-sm flex-shrink-0">
                      <Package size={20} className="text-[#5BCDE9]" />
                    </div>
                ...
                <div className="md:col-span-2">
                  <div className="bg-gray-50 border border-gray-100 rounded-3xl p-6 h-full">
                    <h3 className="font-black text-[10px] uppercase tracking-[0.2em] text-gray-400 mb-6 flex items-center justify-between">
                      ...
                      <span className="text-[#5BCDE9]">{items.length} {t("items_count_label")}</span>
                    </h3>
                      ...
                      {items.map((item) => (
                        <div key={item.id} className="flex justify-between items-start gap-4">
                          <div className="flex-1">
                            ...
                            <p className="text-[10px] text-gray-500 font-bold mt-0.5">{t("qty_label") || "Qty"}: {item.quantity}</p>
                          </div>
                    ...
                    <div className="mt-6 pt-6 border-t border-dashed border-gray-200 space-y-3">
                      <div className="flex justify-between text-[11px] font-bold">
                        <span className="text-gray-500 uppercase tracking-wider">{t("subtotal_label")}</span>
                        <span className="text-[#1A2E35]">{(order.total_amount + order.discount_amount).toFixed(2)} €</span>
                      </div>
                      {order.discount_amount > 0 && (
                        <div className="flex justify-between text-[11px] font-bold">
                          <span className="text-teal-600 uppercase tracking-wider">{t("discount_label")}</span>
                          <span className="text-teal-600">-{order.discount_amount.toFixed(2)} €</span>
                        </div>
                      )}
                      <div className="flex justify-between items-center pt-3 mt-3 border-t border-gray-200">
                        <span className="font-black text-xs text-[#1A2E35] uppercase tracking-widest">{t("total_price_label")}</span>
              ...
              <div className="py-20 text-center">
                ...
                <Link href="/" className="text-[#5BCDE9] font-bold text-sm mt-4 inline-block hover:underline">
                  {t("go_back_home")}
                </Link>
              </div>
            )}
            ...
            <div className="flex flex-col sm:flex-row justify-center gap-4 mt-12 pt-8 border-t border-gray-50">
              <Link
                href="/"
                className="px-10 py-4 rounded-2xl bg-[#5BCDE9] text-white font-black hover:bg-[#38B2D0] transition-all hover:-translate-y-1 shadow-lg shadow-teal-900/10 active:translate-y-0 text-center"
              >
                {t("continue_shopping")}
              </Link>
              <Link
