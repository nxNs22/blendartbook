...
export default function AccountPage() {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) throw error;
      setMessage({ text: t("profile_updated_success"), type: "success" });
    } catch (err: unknown) {
      setMessage({ text: getErrorMessage(err), type: "error" });
    } finally {
      setSaving(false);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12 flex flex-col md:flex-row gap-8">
      ...
      <div className="w-full md:w-64 shrink-0">
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex flex-col items-center mb-8 pb-6 border-b text-center">
            ...
            <div>
              <p className="font-bold text-[#1A2E35] line-clamp-1">{profile.full_name || t("my_account")}</p>
      ...
      <div className="flex-1 bg-white rounded-xl shadow-sm border border-gray-100 p-6 md:p-8">
        {activeTab === "profile" ? (
          // {t("profile_details")} Tab
          <>
            <h2 className="text-2xl font-bold text-[#1A2E35] mb-6 flex items-center gap-2">
              <MapPin className="text-[#2CB391]" /> {t("shipping_contact_details")}
            </h2>
            ...
            <form onSubmit={handleUpdateProfile} className="space-y-6 max-w-2xl">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-xs font-bold text-gray-700 uppercase mb-2">{t("full_name")}</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-700 uppercase mb-2">{t("phone_number")}</label>
                  <input
                    type="tel"
                    value={profile.phone}
              ...
              <div>
                <label className="block text-xs font-bold text-gray-700 uppercase mb-2">{t("delivery_address")}</label>
                <textarea
          ...
          <>
            ...
            {orders.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-gray-500 text-lg">{t("no_orders_yet")}</p>
              </div>
            ) : (
                  ...
                  <div key={order.id} className="border border-gray-200 rounded-lg overflow-hidden hover:shadow-md transition-shadow">
                    ...
                    {expandedOrderId === order.id && (
                      <div className="p-4 md:p-6 border-t border-gray-200 bg-white">
                        <h3 className="font-bold text-gray-800 mb-4">{t("order_items")}</h3>
                          ...
                          {order.items.map((item, idx) => (
                            <div key={idx} className="flex justify-between items-start pb-3 border-b border-gray-100 last:border-b-0">
                              <div className="flex-1">
                                ...
                                <p className="text-xs text-gray-500 mt-1">{t("qty")} {item.quantity}</p>
                              </div>
                        ...
                        <div className="mt-4 p-3 bg-blue-50 rounded-lg">
                          <p className="text-sm text-blue-800"><span className="font-bold">{t("status")}</span> {order.status}</p>
                        </div>
                      </div>
                    )}
                  </div>
