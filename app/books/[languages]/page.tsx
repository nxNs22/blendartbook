...
import { Filter, ChevronDown, Loader2, BookOpen } from "lucide-react";
...
export default function BooksByLanguage() {
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        if (currentLanguage !== 'all') {
          query = query.eq('subcategory', currentLanguage === 'turkish' ? 'turkish' : 'english');
        }
  ...
  const displayTitle = currentLanguage.toLowerCase() === 'all' 
    ? `${t("all")} ${t("books")}` 
    : `${t(currentLanguage.toLowerCase()) || currentLanguage} ${t("books")}`;
    ...
    <div className="bg-[#F9FBF9] min-h-screen pb-20">
      ...
      <div className="bg-white border-b border-gray-100 mb-8">
        <div className="max-w-7xl mx-auto px-6 py-12">
          <nav className="text-[11px] text-gray-400 mb-4 flex items-center gap-2">
            <span>Home</span> <span>/</span> <span>Books</span> <span>/</span> <span className="text-gray-600 font-bold">{displayTitle}</span>
          </nav>
      ...
      <div className="bg-white">
        <div className="max-w-[1400px] mx-auto px-6 py-8">
          ...
          {/* SEARCH, SORT AND PAGINATION BAR */}
          <ListControls 
          ...
          <div className="flex flex-col md:flex-row gap-10">
            ...
            <FilterSidebar 
              products={products}
              defaultBindings={["Paperback", "Hardback", "Board book", "Book", "Sheet", "Spiral bound"]}
            ...
            <main className="flex-1">
              ...
              {!loading && filteredProducts.length === 0 && (
                <div className="text-center py-24 text-gray-400 border-2 border-dashed border-gray-200 rounded-[40px] bg-white">
                  <BookOpen className="mx-auto mb-6 opacity-10" size={64} />
                  <p className="text-lg font-bold">{t("no_products_found") || "No books found"}</p>
                </div>
              )}
            </main>
          </div>
