...
import { Filter, ChevronDown, Loader2, Headphones } from "lucide-react";
import { supabase, getErrorMessage } from "../../lib/supabaseClient"; 
import ProductCard from "../../components/ProductCard";

import { demoAudiobooks } from "../../data/demoProducts";
...
export default function AudiobookCategoryPage() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (filtered.length === 0 && !loading) {
      filtered = demoAudiobooks.filter(product => {
        const langMatch = isAll || product.details?.language?.toLowerCase() === currentLanguage.toLowerCase();
        const priceMatch = product.price <= priceRange;
        return langMatch && priceMatch;
      });
  ...
  const displayTitle = currentLanguage.toLowerCase() === 'all' 
    ? t("all_audiobooks") || "All Audiobooks"
    : `${t(currentLanguage.toLowerCase()) || currentLanguage} ${t("audiobooks") || "Audiobooks"}`;
      ...
      <div className="bg-white border-b border-gray-100 mb-8">
        <div className="max-w-7xl mx-auto px-6 py-12">
          <nav className="text-[11px] text-gray-400 mb-4 flex items-center gap-2">
            <span>Home</span> <span>/</span> <span>{t("audiobooks") || "Audiobooks"}</span> <span>/</span> <span className="text-gray-600 font-bold">{displayTitle}</span>
          </nav>
          
          <div className="flex items-center gap-6">
            <div className="w-16 h-16 bg-purple-50 rounded-full flex items-center justify-center text-purple-600 border border-purple-100 shadow-sm">
               <Headphones size={32} />
            </div>
            <div>
        ...
        <main className="flex-1">
          ...
          {!loading && filteredProducts.length === 0 && (
            <div className="text-center py-24 text-gray-400 border-2 border-dashed border-gray-200 rounded-[40px] bg-white">
              <Headphones className="mx-auto mb-6 opacity-10" size={64} />
              <p className="text-lg font-bold">{t("no_products_found") || "No audiobooks found"}</p>
            </div>
          )}
        </main>
      </div>
